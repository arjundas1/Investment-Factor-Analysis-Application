-- ============================================================
-- User Story 2: Stock Screener - Combined Query
-- Arash Katirai | Investment Factor Analysis Application
--
-- Combines:
-- - Marvin's factor definitions (P/B for value, ROA for
--   profitability, 12-month momentum, PERCENT_RANK normalization)
-- - Arash's user inputs (factor weights, market cap filter)
-- ============================================================

-- NOTE ON DATA:
-- Value      = book-to-market ratio (shareholders_equity / market_cap)
--              Higher = cheaper relative to book value = more "value-like"
-- Profitability = return on assets (net_income / total_assets)
--              Higher = more efficient use of assets
-- Momentum   = 12-month price return (adjusted_close)
--              Higher = stronger recent price trend
-- Size       = market cap (smaller = higher score per size factor theory)
-- All dollar values in Compustat are in millions
-- ============================================================


-- ============================================================
-- QUERY 2 (Complex | Expected runtime: >15s raw, <1s optimized)
-- Factor-Weighted Stock Ranking with Normalized Scores
--
...
-- CASE guards handle NULL/zero in financial columns
raw_metrics AS (
    SELECT
        c.ticker,
        c.company_name,
    ...
    JOIN latest_price lp   ON c.ticker = lp.ticker      -- INNER: drop companies with no price data
    JOIN price_12m_ago p12 ON c.ticker = p12.ticker     -- INNER: drop companies with no 12m history
    WHERE c.market_cap IS NOT NULL
      AND c.market_cap_category = :market_cap_category  -- user input: 'Small', 'Mid', or 'Large'
),
